...
class BulkingController extends Controller
{
    ...
    public function store(
        ...
        try {
            // Cegah pembuatan campaign baru jika ada campaign bulking yang sedang berjalan
            $activeCampaign = Campaign::whereIn('status', [Campaign::STATUS_QUEUED, Campaign::STATUS_RUNNING, Campaign::STATUS_RESTING])->first();
            if ($activeCampaign) {
                throw new \Exception('Sistem tidak dapat membuat campaign baru karena saat ini sedang memproses campaign bulking "' . $activeCampaign->nama . '".');
            }
